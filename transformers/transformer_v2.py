...
def build_vocabulary(spacy_de, spacy_en):
    ...
    #refer https://github.com/harvardnlp/annotated-transformer/issues/130
    ...
    #train, val, test는 iterator 타입입니다.
    #각 요소는 tuple[str, str] 타입이며, 순서대로:
    #source 문장: 독일어 (language_pair[0] = "de")
    #target 문장: 영어 (language_pair[1] = "en")
    ...
    #//즉, **"단어가 어휘 사전에 없을 경우 어떤 인덱스를 반환할지"**를 정해주는 역할을 합니다.    
...
def collate_batch(
    ...
    #print(f"max_padding : {max_padding}, pad_id: {pad_id} ")
    bs_id = torch.tensor([0], device=device)  # <s> token id
    eos_id = torch.tensor([1], device=device)  # </s> token id
    src_list, tgt_list = [], []
    #print(f"batch: {len(batch)}")
    #len(batch) is 32 for example.
    
    #print(f"src_vocab: {src_vocab}")
    for (_src, _tgt) in batch:
        ##print(f"_src : {_src} ")
        ##print(f"src_vocab(src_pipeline(_src)): {src_vocab(src_pipeline(_src))}")
...
# src, trg shape is 32*72 : n_batch x max_pad
def create_dataloaders(
    ...
    #단어(문자열)를 숫자 인덱스로 바꾸는 딕셔너리를 반환합니다.
...
def train_worker(
    ...
    print(f"Train worker process using GPU: {gpu} {ngpus_per_node} for training", flush=True)
...
def train_distributed_model(vocab_src, vocab_tgt, spacy_de, spacy_en, config):
    ...
    mp.spawn(
        train_worker,
        nprocs=ngpus,
        args=(ngpus, vocab_src, vocab_tgt, spacy_de, spacy_en, config, True), join=True,
    )
